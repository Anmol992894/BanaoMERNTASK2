...
// Function to unlike the user
    ...
    const addtweetData = async (e) => {
        e.preventDefault();
        const requestedData= {Content:tweet, Image: file}
        const response=await axios.post(`${API_BASE_URL}/createpost/${users._id}`,requestedData)
        .then((data)=>{
            console.log(data);
        })
        .catch((error)=>{
    ...
    const tweetdelete = async (id) => {
        const response = await axios.delete(`${API_BASE_URL}/deletepost/${id}`);
    ...
    const alltweet = async () => {
        ...
        if (response.status === 200) {
            setAlltwet(response.data.posts)
        } else {
            Swal.fire({
                icon: 'error',
                            ...
                            <div className='col-12 my-2' >
                                <div className="card w-100 d-flex flex-column" >
                                    ...
                                    <div onClick={() => gototweetpage(data._id)} className='d-flex mt-3 flex-column align-items-center justify-content-center ps-3'>
                                        {data.Image != "None" && <img style={{ height: "300px", width: "100%" }} src={data.Image} alt='Tweet Image' />}
                    ...
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <div className='col-12'>
                                ...
                                <input placeholder='Provide Image Link' name="text" type="text" id="drop_zone" onChange={(e) => setFile(e.target.value)} />
                            </div>
                        </div>
